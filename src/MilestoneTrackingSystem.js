...
export const SESSION_MILESTONES = {
  ...
  "documentation-expert": {
    ...
    requirementFunction: (tracking) => {
      const notebookCount =
        tracking.totals?.["engineering-notebook"]?.count || 0;
...
export const LIFETIME_ACHIEVEMENTS = {
  ...
  "txr-achiever": {
    ...
    requirementFunction: (
      ...
      Object.entries(milestoneHistory || {}).forEach(
  ...
  "txr-veteran": {
    ...
    requirementFunction: (
      ...
      return {
        ...
        percentage: earned
          ? 100
          : Math.min(100, (completedSessions / 10) * 100),
      };
    },
    xp: 200,
  },
...
export const checkAllMilestones = (
  ...
  if (attendance[currentSession] && sessionInfo?.classDates) {
    const attendanceData = {
      ...
      absent: 0,
      late: 0,
      total: sessionInfo.classDates.length,
    };
  ...
  Object.values(SESSION_MILESTONES).forEach((milestone) => {
    ...
    if (result.earned) {
      const alreadyEarned = studentMilestones[studentId]?.[
        currentSession
...
export const getMilestoneProgress = (
  ...
  Object.entries(SESSION_MILESTONES).forEach(([id, milestone]) => {
    const result = milestone.requirementFunction(tracking, sessionInfo);
    const alreadyEarned =
      studentMilestones[studentId]?.[currentSession]?.includes(id);
  ...
  Object.entries(LIFETIME_ACHIEVEMENTS).forEach(([id, achievement]) => {
    ...
    const alreadyEarned =
      student.achievements?.some((achievementId) => {
